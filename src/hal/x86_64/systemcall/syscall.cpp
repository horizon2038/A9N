...
namespace a9n::hal::x86_64
{
    ...
    void init_syscall()
    {
        ...
        _write_msr(
            msr::EFER,
            (_read_msr(msr::EFER) | efer_flag::SYSCALL_EXTENSION
             | efer_flag::NO_EXECUTE)
        );

        // mask eflags
        // TODO: remove magic number
        _write_msr(msr::SFMASK, 0x200);
    ...
    )
    {
        ...
        a9n::kernel::utility::logger::printk(
            "descriptor : 0x%016llx [ %02llx ]\n",
            descriptor,
            depth
        );
        a9n::kernel::utility::logger::printk("tag : 0x%016llx\n", tag);
        a9n::kernel::utility::logger::printk(
            "message_length : 0x%016llx\n",
            message_length
        );
    }
}
