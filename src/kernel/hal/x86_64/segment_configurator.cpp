...
extern "C" void _load_gdt_asm(uint16_t size, uint64_t *offset);
extern "C" void _load_segment_register_asm(uint16_t code_segment_register);
...
namespace hal::x86_64
{
    ...
    {
        this->load_gdt();
        this->load_segment_register(0x08);
    }
    ...
    void segment_configurator::load_gdt()
    {
        ...
        _load_gdt_asm(gdt_size, gdt_address);
    }

    void segment_configurator::load_segment_register(uint16_t code_segment_register)
    {
        _load_segment_register_asm(code_segment_register);
    }
}
