    ...
    struct kernel_object
    {
      ...
      private:
        constexpr static uint64_t memory_manager_size
            = sizeof(kernel::memory_manager);

        constexpr static uint64_t interrupt_manager_size
            = sizeof(kernel::interrupt_manager);

        constexpr static uint64_t process_manager_size
            = sizeof(kernel::process_manager);

        constexpr static uint64_t ipc_manager_size
            = sizeof(kernel::ipc_manager);
    };
}
