...
EFI_STATUS load_kernel(EFI_FILE_PROTOCOL *kernel, uint64_t offset)
{
    ...
    while(!EFI_ERROR(efi_status))
    {
        ...
        efi_status = load_elf_segment(loaded_elf64_header, loaded_elf64_program_header_table, 0);
        break;
    }
    return efi_status;
}
...
EFI_STATUS locate_elf_segment(elf64_header *header, elf64_program_header *program_header)
{
    ...
    efi_status = gBS->AllocatePages(AllocateAnyPages, EfiLoaderData, EFI_SIZE_TO_PAGES(program_header->memory_size), (EFI_PHYSICAL_ADDRESS*)&program_header->virtual_address);
