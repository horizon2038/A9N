...
namespace liba9n
{
    ...
    class option
    {
      ...
      public:
        ...
        constexpr option([[maybe_unused]] option_none_tag) noexcept : dummy {}, is_some_flag(false)
        {
            // lazy initialization
        }
        ...
        constexpr option(const option &other) noexcept : is_some_flag(other.is_some())
        {
            if (other.is_none())
            {
                return;
        ...
        constexpr option(option &&other) noexcept : is_some_flag(other.is_some())
        {
            if (other.is_none())
            {
                return;
        ...
        constexpr option(const option<U> &other) noexcept : is_some_flag(other.is_some())
        {
            if (other.is_none())
            {
                return;
        ...
        template<typename U>
            requires(!liba9n::std::is_reference_v<U> && liba9n::std::is_convertible_v<U, T>)
        constexpr option(option<U> &&other) noexcept : is_some_flag(other.is_some())
        {
            if (other.is_none())
            {
                return;
        ...
        template<typename U>
            requires liba9n::std::is_copy_constructible_v<T> && liba9n::std::is_convertible_v<U &&, T>
        ...
        template<typename U>
            requires liba9n::std::is_move_constructible_v<T> && liba9n::std::is_convertible_v<U, T>
        constexpr T unwrap_or(U &&instead_some_value) &&
        {
            if (is_some())
            {
        ...
        template<typename Function, typename Tcvref = T &, typename U = liba9n::std::invoke_result_t<Function, Tcvref>>
        ...
        template<typename Function, typename Tcvref = T &, typename U = liba9n::std::invoke_result_t<Function, Tcvref>>
            requires(
                !liba9n::std::is_same_v<U, option_in_place_tag>
                && !liba9n::std::is_same_v<U, option_none_tag>
            )
        constexpr auto transform(Function &&function) &
        {
            ...
            return option<U>(liba9n::std::invoke(liba9n::std::forward<Function>(function), unwrap()));
        }
