...
#let document_status(version) = [
  ...
  #reference_table(
    ...
    [実装Platform], [`x86_64/pc99`，`aarch64/qemu`（QEMU `virt`），`aarch64/rpi4b`（Raspberry Pi 4 Model B）．Spencerはx86_64のExecution BackendとしてQEMUを選択できる．`rpi4b`はVideoCore FirmwareとU-Bootを経由する実機Bootおよび4 Core SMP Bootを確認している．],
    [Multiprocessing], [`A9N_CONFIG_ENABLE_SMP`でBuild時に選択する．x86_64はACPIとLocal APIC，`aarch64/qemu`はDTBとPSCI，`aarch64/rpi4b`はDTB Spin Tableを用いてApplication Processorを起動する．QEMU `virt`とRaspberry Pi 4 Model Bの4 Core起動を検証済みである．Kernel EntryはGiant Lockで直列化する．],
