...
#let document_status(version) = [
  ...
  #reference_table(
    ...
    [実装Platform], [`x86_64/qemu`（PC99），`aarch64/qemu`（QEMU `virt`），`aarch64/rpi4b`（Raspberry Pi 4 Model B）．`rpi4b`はVideoCore FirmwareとU-Bootを経由する実機Bootおよび4 Core SMP Bootを確認している．],
