...
namespace a9n::kernel
{
    ...
    void interrupt_manager::init_handler(void)
    {
        ...
        a9n::kernel::utility::logger::printk("Registering IPI reschedule handler ...\n");
    ...
    extern "C" void handle_ipi_reschedule(void)
    {
        ...
        if (!result) [[unlikely]]
        {
            DEBUG_LOG("Failed to reschedule from IPI: %s", kernel_error_to_string(result.unwrap_error()));
        }
    }
    ...
    )
    {
        ...
        current_process_manager().and_then(
            [=](process_manager *manager) -> kernel_result
            {
                return manager->retrieve_current_process()
                    .transform_error(convert_kernel_to_capability_error)
                    .and_then(
                        [=](process *current_process) -> capability_result
                        {
                    ...
                    .or_else(
                        [=](capability_error e) -> kernel_result
                        {
                            ...
                            a9n::kernel::utility::logger::printk(
                                "Fault : %s, fault_code : %lld, arch_fault_code: %llu, "
                                "fault_address : 0x%016llx\n",
                                a9n::kernel::fault_type_to_string(type),
                                fault_code,
                                arch_fault_code,
