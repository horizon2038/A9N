...
Version 0.1.15のSPENCER CLIは，x86_64 Architectureに対して`--platform qemu`を受理する．Getting Startedでは各Interfaceの内部を変更せず，標準構成のBootを先に確認する．生成したx86_64 Disk Imageは，QEMUだけでなくUEFI実機でも起動できる．Capabilityを用いたServiceの構成は「Building Init and Services」，Register配置や独自Entryの実装は「x86_64 ABI」に記載する．
